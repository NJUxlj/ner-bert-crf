...
class Fence:
    ...
    #用行列（行序号，列序号）组成的list代表一个节点，每两个相邻的列的节点之间可以计算距离
    #e.g:node1 = [2,1] node2 = [3, 2]
    #为两个节点给一个固定的路径分值
    def score(self, node1:str, node2:str):
        if node1 == "start":
...
def beam_search(fence, beam_size):
    width = fence.width
    height = fence.height
    starter = Path() # 多个节点组成的路径
    beam_buffer = [starter] # 存储不同的路径
    new_beam_buffer = []
    while True:
        for path in beam_buffer:
            path_length = len(path) - 1 # start 不能包含
            for h in range(height): # 每个节点有h种标签选择
                node = [h, path_length]
